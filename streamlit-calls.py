...
def load_and_preprocess_data(xls_like):
    ...
    # If 'week' column is a Period, stringify it to avoid plotting/type issues
    ...
    # Duration columns can be numeric or HH:MM:SS strings — normalize to seconds
    duration_cols = [
    ...
    for col in duration_cols:
        if col in phone_data.columns:
            s = phone_data[col]
            # If it's not numeric, try timedelta first
            if not pd.api.types.is_numeric_dtype(s):
                td = pd.to_timedelta(s, errors='coerce')
                # If timedelta worked for at least some rows, use seconds
                if td.notna().any():
    ...
    # Convert email counters to numeric if present
...
def main():
    ...
    # File upload (REQUIRED – no local fallback)
    uploaded_file = st.file_uploader("Upload Excel file", type=['xlsx'])

    if uploaded_file is None:
        st.warning("Please upload an .xlsx containing sheets 'Phone Data' and 'Case Data'.")
        st.stop()

    # Load data
    with st.spinner('Loading and preprocessing data...'):
        phone_data, case_data = load_and_preprocess_data(uploaded_file)
    ...
    with tab4:
        # Build daily_combined on the fly
        phone_daily = phone_data.groupby('Call Date').size().reset_index()
        phone_daily.columns = ['Date', 'Phone_Volume']
        phone_daily['Date'] = pd.to_datetime(phone_daily['Date'])
